...
# This script is a modified version of the original jupynote.py script
# Example of use:
# python .\jupynotex.py ..\ICH1R.ipynb '-' '70'
...
VERBATIM_END = [r"\end{verbatim}", r"\end{footnotesize}", r""]
# VERBATIM_BEGIN = [r"\begin{verbatim}"]
# VERBATIM_END = [r"\end{verbatim}", r""]
...
class ItemProcessor:
    ...
    def get_item_data(self, item, cell_idx):
        ...
        for func in functions:
            content = func(self, content, cell_idx)

        return content

    def process_plain_text(self, lines, cell_idx):
        """Process plain text."""
        return _process_plain_text(lines, self.config_options)

    def process_png(self, image_data, cell_idx):
        """Process a PNG: just save the received b64encoded data to a temp file."""
        # _, fname = tempfile.mkstemp(suffix='.png')
        print(cell_idx)
        jupynotex_figpath = './fig/' 
        fname = notebook_path.replace('\\','').replace('.','') + '_cell{:02d}out'.format(cell_idx) + '.png'
        with open(jupynotex_figpath + fname, 'wb') as fh:
            fh.write(base64.b64decode(image_data))
        return fname

    def process_svg(self, image_data, cell_idx):
    ...
    def include_graphics(self, fname, cell_idx):
        ...
        return r"\begin{{center}}\includegraphics[width=0.68\textwidth]{{{}}}\end{{center}}".format(fname_no_backslashes)

    def listwrap(self, item, cell_idx):
...
class Notebook:
    ...
    def _proc_out(self, content, cell_idx):
        ...
        for item in outputs:
            ...
            if output_type in ('execute_result', 'display_data'):
                more_content = processor.get_item_data(item, cell_idx)
            elif output_type == 'stream':
                more_content = processor.process_plain_text(item["text"], cell_idx)
    ...
    def get(self, cell_idx):
        ...
        output = self._proc_out(content, cell_idx)
...
def main(notebook_path, cells_spec, config_options):
    ...
    output_file = open('./tex/' + notebook_path.replace('\\','').replace('.','') + '_' + cells_spec +'.tex', "w", encoding='utf8')
    ...
    for cell in cells:
        ...
        print(r"\begin{{lstlisting}}[language=Python, caption=Súbor \lstinline|{} cell:{:02d}|]".format(notebook_path.replace('\\','/'), cell))
        print(src + r" \end{lstlisting}")

        # output_file.write(r"\vbox{%")
        output_file.write('\n')
        output_file.write(r"\begin{{lstlisting}}[language=Python, caption=Súbor \lstinline|{} cell:{:02d}|]".format(notebook_path.replace('\\','/'), cell))
        output_file.write('\n')
        output_file.write(src)
        output_file.write(r" \end{lstlisting}")
        output_file.write('\n')

        if out:
            print(out)
            output_file.write(out)
        # output_file.write(r"}%end of vbox")
        output_file.write('\n')
